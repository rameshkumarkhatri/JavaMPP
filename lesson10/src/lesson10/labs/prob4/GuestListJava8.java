...
public class GuestListJava8 {
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		//implement
		invited.stream().filter(guest ->checkLegalHandler(guest)
		).filter(guest -> !guest.getStatus().equals(Status.ILLEGAL))
				.map(guest -> guest.getName()).collect(Collectors.toList());
	}

	private boolean checkLegalHandler(Guest guest) {
		try {
